...
public class TriangleMaskPlacer : MonoBehaviour
{
    ...
    /// </summary>
    public SpriteMask PlaceMaskWithObstacle(BoxCollider2D obstacle)
    {
        ...
        mask.transform.localScale = new Vector3(sx * 0.7f , sy * mirrorY * 1.5f, 1f);
        ...
        // 已有：Bmid, Bdir(单位向量), Blen, yAxisWorld(单位向量, 指向源侧或其镜像), sx, heightScale, mirrorY
        // 求 Sprite 的“本地高度”并换算为世界高度
        float spriteHeight = (mask.sprite != null) ? (mask.sprite.rect.height / mask.sprite.pixelsPerUnit) : 1f;
        float heightWorld = Mathf.Abs(sx * heightScale) * spriteHeight * (mirrorAboutLocalX ? -1f : 1f);
        // mirror 后顶点应在对边某侧；用符号控制朝向

        Vector2 baseLeft = Bmid - Bdir * (Blen * 0.5f);
        Vector2 baseRight = Bmid + Bdir * (Blen * 0.5f);
        Vector2 apex = Bmid + yAxisWorld * heightWorld;

        // 写入 TriangleMaskArea
        var area = mask.GetComponent<TriangleMaskArea>();
        if (area == null) area = mask.gameObject.AddComponent<TriangleMaskArea>();
        area.SetTriangleWorld(baseLeft, baseRight, apex);
